...
def aer_activation_stencil(
    ...
    with computation(PARALLEL), interval(...):
        ...
        if (tk >= (constants.MAPL_TICE - 40.0)) and (plo > 10000.0) and (0.1 < wupdraft) and (wupdraft < 100.0):
            ...
            wpe = exp(float64(77.34491296) - float64(7235.424651) / tk - float64(8.2) * log(tk) + tk * float64(5.7113e-3))  # [pa]
            dumw = sqrt(TWOPI * WMOLMASS / RGASJMOL / tk)  # [s/m]
            dvprime = dv / ((rdrp / (rdrp + DELTAV)) + (dv * dumw / (rdrp * ALPHAC)))  # [m^2/s] - eq. (17)
            xka = (float64(5.69) + float64(0.017) * (tk - float64(273.15))) * float64(418.4e-5)  # [j/m/s/k] (0.0238 j/m/s/k at 273.15 k)
            duma = sqrt(TWOPI * AMOLMASS / RGASJMOL / tk)  # [s/m]
            xkaprime = xka / ((rdrp / (rdrp + DELTAT)) + (xka * duma / (rdrp * ALPHAT * DENH2O * CPAIR)))  # [j/m/s/k]
            g = float64(1.0) / (
                (DENH2O * RGASJMOL * tk) / (wpe * dvprime * WMOLMASS) + ((HEATVAP * DENH2O) / (xkaprime * tk)) * ((HEATVAP * WMOLMASS) / (RGASJMOL * tk) - float64(1.0))
            )  # [m^2/s]
            a = (float64(2.0) * surten * WMOLMASS) / (DENH2O * RGASJMOL * tk)  # [m]
            alpha = (GRAVITY / (RGASJMOL * tk)) * ((WMOLMASS * HEATVAP) / (CPAIR * tk) - AMOLMASS)  # [1/m]
            gamma = (RGASJMOL * tk) / (wpe * WMOLMASS) + (WMOLMASS * HEATVAP * HEATVAP) / (CPAIR * plo * AMOLMASS * tk)  # [m^3/kg]
            ...
            while n < constants.N_MODES:
                ...
                smax = float64(0.0)  # [1]
                sm = (float64(2.0) / sqrt(bibar[0, 0, 0][n])) * (a / (3.0 * rg[0, 0, 0][n])) ** float64(1.5)  # [1]
                eta = dum**3 / (TWOPI * DENH2O * gamma * ni[0, 0, 0][n])  # [1]
                f1 = float64(0.5) * exp(2.50 * xlogsigm**2)  # [1]
                f2 = float64(1.0) + 0.25 * xlogsigm  # [1]
                smax = smax + (f1 * (zeta / eta) ** float64(1.5) + f2 * (sm**2 / (eta + float64(3.0) * zeta)) ** float64(0.75)) / sm**2  # [1] - eq. (6)
                n += 1
...
class AerActivation(NDSLRuntime):
    ...
    def __init__(
        ...
        self._nact = self.make_local(quantity_factory, [I_DIM, J_DIM, K_DIM, "n_modes"])
        self._ni = self.make_local(quantity_factory, [I_DIM, J_DIM, K_DIM, "n_modes"])
        self._rg = self.make_local(quantity_factory, [I_DIM, J_DIM, K_DIM, "n_modes"])
        self._sig0 = self.make_local(quantity_factory, [I_DIM, J_DIM, K_DIM, "n_modes"])
        self._bibar = self.make_local(quantity_factory, [I_DIM, J_DIM, K_DIM, "n_modes"])
