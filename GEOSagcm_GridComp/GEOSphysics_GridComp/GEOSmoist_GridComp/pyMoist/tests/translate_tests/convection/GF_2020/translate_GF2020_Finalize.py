...
class TranslateGF2020_Finalize(TranslateFortranData2Py):
    ...
    def compute(self, inputs):
        ...
        state.specific_humidity_surface.field[:] = self.unmodified_state[
            "specific_humidity_surface_bugworkaroundname"
        ]
        ...
        state.convective_cloud_fraction.field[:] = self.unmodified_state[
            "convective_cloud_fraction_bugworkaroundname"
        ]
        ...
        state.large_scale_cloud_fraction.field[:] = self.unmodified_state[
            "large_scale_cloud_fraction_bugworkaroundname"
        ]
        state.p_interface_timestep_start.field[:] = self.unmodified_state[
            "p_interface_timestep_start_bugworkaroundname"
        ]
        ...
        state.geopotential_height_interface.field[:] = self.unmodified_state[
            "geopotential_height_interface_bugworkaroundname"
        ]
        state.geopotential_height_surface.field[:] = self.unmodified_state[
            "geopotential_height_surface_bugworkaroundname"
        ]
        ...
        state.dspecific_humiditydt_pbl.field[:] = self.unmodified_state[
            "dspecific_humiditydt_pbl_bugworkaroundname"
        ]
        ...
        state.dvapordt_from_dynamics.field[:] = self.unmodified_state[
            "dvapordt_from_dynamics_bugworkaroundname"
        ]
        ...
        cumulus_parameterization_state.input.grid_scale_forcing_t.field[:] = self.manual_inputs[
            "grid_scale_forcing_t"
        ]
        cumulus_parameterization_state.input.grid_scale_forcing_vapor.field[:] = self.manual_inputs[
            "grid_scale_forcing_vapor"
        ]
        cumulus_parameterization_state.input.subgrid_scale_forcing_t.field[:] = self.manual_inputs[
            "subgrid_scale_forcing_t"
        ]
        cumulus_parameterization_state.input.subgrid_scale_forcing_vapor.field[:] = self.manual_inputs[
            "subgrid_scale_forcing_vapor"
        ]
        cumulus_parameterization_state.input.seed_convection.field[:] = self.manual_inputs["seed_convection"]
        cumulus_parameterization_state.input.saturation_water_vapor.field[:] = self.manual_inputs[
            "saturation_water_vapor"
        ]
        cumulus_parameterization_state.input.ocean_fraction.field[:] = self.manual_inputs["ocean_fraction"]
        cumulus_parameterization_state.input.convection_fraction.field[:] = self.manual_inputs[
            "convection_fraction"
        ]
        cumulus_parameterization_state.input.surface_type.field[:] = self.manual_inputs["surface_type"]
        cumulus_parameterization_state.input.lateral_entrainment_rate.field[:] = self.manual_inputs[
            "lateral_entrainment_rate"
        ]
        ...
        cumulus_parameterization_state.output.dvapordt.field[:] = self.manual_inputs["dvapordt"][
            :, :, :, [1, 2, 0]
        ]
        cumulus_parameterization_state.output.dcloudicedt.field[:] = self.manual_inputs["dcloudicedt"][
            :, :, :, [1, 2, 0]
        ]
        ...
        cumulus_parameterization_state.output.dnliquiddt.field[:] = self.manual_inputs["dnliquiddt"][
            :, :, :, [1, 2, 0]
        ]
        cumulus_parameterization_state.output.dnicedt.field[:] = self.manual_inputs["dnicedt"][
            :, :, :, [1, 2, 0]
        ]
        cumulus_parameterization_state.output.dbuoyancydt.field[:] = self.manual_inputs["dbuoyancydt"][
            :, :, :, [1, 2, 0]
        ]
        cumulus_parameterization_state.output.dconvectiveicedt.field[:] = self.manual_inputs[
            "dconvectiveicedt"
        ][:, :, :, [1, 2, 0]]
        cumulus_parameterization_state.output.dlargescaleicedt.field[:] = self.manual_inputs[
            "dlargescaleicedt"
        ][:, :, :, [1, 2, 0]]
        cumulus_parameterization_state.output.dconvectiveliquiddt.field[:] = self.manual_inputs[
            "dconvectiveliquiddt"
        ][:, :, :, [1, 2, 0]]
        cumulus_parameterization_state.output.dlargescaleliquiddt.field[:] = self.manual_inputs[
            "dlargescaleliquiddt"
        ][:, :, :, [1, 2, 0]]
        cumulus_parameterization_state.output.dconvectivecloudfractiondt.field[:] = self.manual_inputs[
            "dconvectivecloudfractiondt"
        ][:, :, :, [1, 2, 0]]
        cumulus_parameterization_state.output.dlargescalecloudfractiondt.field[:] = self.manual_inputs[
            "dlargescalecloudfractiondt"
        ][:, :, :, [1, 2, 0]]
        cumulus_parameterization_state.output.error_code.field[:] = self.manual_inputs["error_code"][
            :, :, [1, 2, 0]
        ]
        cumulus_parameterization_state.output.downdraft_origin_level.field[:] = (
            self.manual_inputs["downdraft_origin_level"][:, :, [1, 2, 0]] - 1
        )
        cumulus_parameterization_state.output.lcl_level.field[:] = (
            self.manual_inputs["lcl_level"][:, :, [1, 2, 0]] - 1
        )
        cumulus_parameterization_state.output.updraft_origin_level.field[:] = (
            self.manual_inputs["updraft_origin_level"][:, :, [1, 2, 0]] - 1
        )
        cumulus_parameterization_state.output.updraft_lfc_level.field[:] = (
            self.manual_inputs["updraft_lfc_level"][:, :, [1, 2, 0]] - 1
        )
        cumulus_parameterization_state.output.cloud_top_level.field[:] = (
            self.manual_inputs["cloud_top_level"][:, :, [1, 2, 0]] - 1
        )
        ...
        cumulus_parameterization_state.output.cloud_base_mass_flux_modified.field[:] = self.manual_inputs[
            "cloud_base_mass_flux_modified"
        ][:, :, [1, 2, 0]]
        ...
        cumulus_parameterization_state.output.p_cloud_levels_forced.field[:] = self.manual_inputs[
            "p_cloud_levels_forced"
        ][:, :, :, [1, 2, 0]]
        cumulus_parameterization_state.output.entrainment_rate.field[:] = self.manual_inputs[
            "entrainment_rate"
        ][:, :, :, [1, 2, 0]]
        cumulus_parameterization_state.output.mass_entrainment_updraft_forced.field[:] = self.manual_inputs[
            "mass_entrainment_updraft_forced"
        ][:, :, :, [1, 2, 0]]
        cumulus_parameterization_state.output.mass_entrainment_downdraft_forced.field[:] = self.manual_inputs[
            "mass_entrainment_downdraft_forced"
        ][:, :, :, [1, 2, 0]]
        cumulus_parameterization_state.output.mass_detrainment_updraft_forced.field[:] = self.manual_inputs[
            "mass_detrainment_updraft_forced"
        ][:, :, :, [1, 2, 0]]
        ...
        cumulus_parameterization_state.output.evaporate_in_downdraft_forced.field[:] = self.manual_inputs[
            "evaporate_in_downdraft_forced"
        ][:, :, :, [1, 2, 0]]
        cumulus_parameterization_state.output.cloud_liquid_after_rain_forced.field[:] = self.manual_inputs[
            "cloud_liquid_after_rain_forced"
        ][:, :, :, [1, 2, 0]]
        cumulus_parameterization_state.output.t_updraft.field[:] = self.manual_inputs["t_updraft"][
            :, :, :, [1, 2, 0]
        ]
        cumulus_parameterization_state.output.convective_cloud_fraction.field[:] = self.manual_inputs[
            "convective_cloud_fraction_output"
        ][:, :, :, [1, 2, 0]]
        cumulus_parameterization_state.output.cloud_workfunction_0.field[:] = self.manual_inputs[
            "cloud_workfunction_0"
        ]
        cumulus_parameterization_state.output.cloud_workfunction_1.field[:] = self.manual_inputs[
            "cloud_workfunction_1"
        ]
        cumulus_parameterization_state.output.cloud_workfunction_2.field[:] = self.manual_inputs[
            "cloud_workfunction_2"
        ]
        cumulus_parameterization_state.output.cloud_workfunction_3.field[:] = self.manual_inputs[
            "cloud_workfunction_3"
        ]
        cumulus_parameterization_state.output.cloud_workfunction_1_pbl.field[:] = self.manual_inputs[
            "cloud_workfunction_1_pbl"
        ]
        cumulus_parameterization_state.output.cloud_workfunction_1_cin.field[:] = self.manual_inputs[
            "cloud_workfunction_1_cin"
        ]
        cumulus_parameterization_state.output.cape_removal_time_scale.field[:] = self.manual_inputs[
            "cape_removal_time_scale"
        ]
        cumulus_parameterization_state.output.pbl_time_scale.field[:] = self.manual_inputs["pbl_time_scale"]
        cumulus_parameterization_state.output.lightning_density.field[:] = self.manual_inputs[
            "lightning_density"
        ]
        cumulus_parameterization_state.output.evaporation_sublimation_tendency.field[:] = self.manual_inputs[
            "evaporation_sublimation_tendency"
        ]
        cumulus_parameterization_state.output.convective_precip_flux.field[:] = self.manual_inputs[
            "convective_precip_flux"
        ]
        ...
        cumulus_parameterization_state.input_output.latent_heat_flux.field[:] = self.manual_inputs[
            "latent_heat_flux"
        ]
        cumulus_parameterization_state.input_output.longitude_degrees.field[:] = self.manual_inputs[
            "longitude_degrees"
        ]
        cumulus_parameterization_state.input_output.latitude_degrees.field[:] = self.manual_inputs[
            "latitude_degrees"
        ]
        ...
        cumulus_parameterization_state.input_output.t_modified_by_advection.field[:] = self.manual_inputs[
            "t_modified_by_advection"
        ]
        cumulus_parameterization_state.input_output.vapor_modified_by_advection.field[:] = self.manual_inputs[
            "vapor_modified_by_advection"
        ]
        cumulus_parameterization_state.input_output.geopotential_height_forced.field[:] = self.manual_inputs[
            "geopotential_height_forced"
        ]
        ...
        cumulus_parameterization_state.input_output.convective_scale_velocity.field[:] = self.manual_inputs[
            "convective_scale_velocity"
        ]
        cumulus_parameterization_state.input_output.buoyancy_excess.field[:] = self.manual_inputs[
            "buoyancy_excess"
        ]
        cumulus_parameterization_state.input_output.large_scale_ice.field[:] = self.manual_inputs[
            "large_scale_ice"
        ]
        cumulus_parameterization_state.input_output.convective_ice.field[:] = self.manual_inputs[
            "convective_ice"
        ]
        cumulus_parameterization_state.input_output.large_scale_liquid.field[:] = self.manual_inputs[
            "large_scale_liquid"
        ]
        cumulus_parameterization_state.input_output.convective_liquid.field[:] = self.manual_inputs[
            "convective_liquid"
        ]
        cumulus_parameterization_state.input_output.large_scale_cloud_fraction.field[:] = self.manual_inputs[
            "large_scale_cloud_fraction"
        ]
        cumulus_parameterization_state.input_output.convective_cloud_fraction.field[:] = self.manual_inputs[
            "convective_cloud_fraction"
        ]
        cumulus_parameterization_state.input_output.chemistry_tracers.field[:] = self.manual_inputs[
            "chemistry_tracers"
        ]
        chemistry_tracers_input_5d = np.full(
            cumulus_parameterization_state.input_output.chemistry_tracers_output.field[:].shape, np.nan
        )
