...
class TranslateGFDL_1M(TranslateFortranData2Py):
    def __init__(
        ...
        if MICRO_BENCH:
            self._ts = 10
            stencil_factory.config.dace_config.set_timer(stencil_factory.comm)
        else:
            self._ts = 0
    ...
    def compute(self, inputs):
        ...
        safe_assign_array(state.precipitation_at_surface.deep_convective_precipitation.field[:], inputs["precipitation_at_surface_deep_convective_precipitation"])
        safe_assign_array(state.precipitation_at_surface.anvil_precipitation.field[:], inputs["precipitation_at_surface_anvil_precipitation"])
        safe_assign_array(state.precipitation_at_surface.shallow_convective_precipitation.field[:], inputs["precipitation_at_surface_shallow_convective_precipitation"])
        safe_assign_array(state.precipitation_at_surface.deep_convective_snow.field[:], inputs["precipitation_at_surface_deep_convective_snow"])
        safe_assign_array(state.precipitation_at_surface.anvil_snow.field[:], inputs["precipitation_at_surface_anvil_snow"])
        safe_assign_array(state.precipitation_at_surface.shallow_convective_snow.field[:], inputs["precipitation_at_surface_shallow_convective_snow"])
        # safe_assign_array(state.lcl_height.field[:] , inputs["lcl_height"])
        ...
        safe_assign_array(
            state.covariance_liquid_water_static_energy_and_total_water_specific_humidity.field[:],
            inputs["covariance_liquid_water_static_energy_and_total_water_specific_humidity"],
        )
        safe_assign_array(state.omega.field[:], inputs["omega"])
        safe_assign_array(state.pdf_first_plume_fractional_area.field[:], inputs["pdf_first_plume_fractional_area"])
        safe_assign_array(state.vertical_motion.velocity.field[:], inputs["vertical_motion_velocity"])
        safe_assign_array(state.vertical_motion.variance.field[:], inputs["vertical_motion_variance"])
        safe_assign_array(state.vertical_motion.third_moment.field[:], inputs["vertical_motion_third_moment"])
        safe_assign_array(state.liquid_water_static_energy.flux.field[:], inputs["liquid_water_static_energy_flux"])
        safe_assign_array(state.liquid_water_static_energy.variance.field[:], inputs["liquid_water_static_energy_variance"])
        safe_assign_array(state.liquid_water_static_energy.third_moment.field[:], inputs["liquid_water_static_energy_third_moment"])
        safe_assign_array(state.total_water.flux.field[:], inputs["total_water_flux"])
        safe_assign_array(state.total_water.variance.field[:], inputs["total_water_variance"])
        ...
        safe_assign_array(state.cloud_ice_sublimation.field[:], inputs["cloud_ice_sublimation"])
        safe_assign_array(state.icefall.field[:], inputs["icefall"])
        safe_assign_array(state.freezing_rainfall.field[:], inputs["freezing_rainfall"])
        safe_assign_array(state.relative_humidity_after_pdf.field[:], inputs["relative_humidity_after_pdf"])
        safe_assign_array(state.buoyancy_flux.field[:], inputs["buoyancy_flux"])
        safe_assign_array(state.liquid_water_flux.field[:], inputs["liquid_water_flux"])
        safe_assign_array(state.hydrostatic_pdf_iterations.field[:], inputs["hydrostatic_pdf_iterations"])
        safe_assign_array(state.radiation_field.cloud_fraction.field[:], inputs["radiation_field_cloud_fraction"])
        safe_assign_array(state.radiation_field.vapor.field[:], inputs["radiation_field_vapor"])
        safe_assign_array(state.radiation_field.liquid.field[:], inputs["radiation_field_liquid"])
        safe_assign_array(state.radiation_field.ice.field[:], inputs["radiation_field_ice"])
        safe_assign_array(state.radiation_field.rain.field[:], inputs["radiation_field_rain"])
        safe_assign_array(state.radiation_field.snow.field[:], inputs["radiation_field_snow"])
        safe_assign_array(state.radiation_field.graupel.field[:], inputs["radiation_field_graupel"])
        safe_assign_array(state.cloud_particle_effective_radius.liquid.field[:], inputs["cloud_particle_effective_radius_liquid"])
        safe_assign_array(state.cloud_particle_effective_radius.ice.field[:], inputs["cloud_particle_effective_radius_ice"])
        safe_assign_array(state.precipitation_at_surface.rain.field[:], inputs["precipitation_at_surface_rain"])
        safe_assign_array(state.precipitation_at_surface.snow.field[:], inputs["precipitation_at_surface_snow"])
        safe_assign_array(state.precipitation_at_surface.ice.field[:], inputs["precipitation_at_surface_ice"])
        safe_assign_array(state.precipitation_at_surface.graupel.field[:], inputs["precipitation_at_surface_graupel"])
        safe_assign_array(state.non_anvil_large_scale.precip.field[:], inputs["non_anvil_large_scale_precip"])
        safe_assign_array(state.non_anvil_large_scale.snow.field[:], inputs["non_anvil_large_scale_snow"])
        ...
        safe_assign_array(state.non_anvil_large_scale.liquid_precip_flux.field[:], inputs["non_anvil_large_scale_liquid_precip_flux"])
        safe_assign_array(state.non_anvil_large_scale.ice_precip_flux.field[:], inputs["non_anvil_large_scale_ice_precip_flux"])
        ...
        # safe_assign_array(state.radar.echo_top_reflectivity.field[:] , inputs["radar_echo_top_reflectivity"])
        # safe_assign_array(state.radar.minus_10c_reflectivity.field[:] , inputs["radar_minus_10c_reflectivity"])
        ...
        validation_outputs = copy.deepcopy(outputs)
        ...
        if self._ts > 0:
            ...
            for _ in range(self._ts):
                code(state=state)
            e = time.perf_counter()
            ndsl_log.info(f"GFDL1M Driver micro bench: {(e - s) / self._ts:.4f}s")
            self.stencil_factory.config.dace_config.performance_collector.write_out_rank_0(self.stencil_factory.backend, float(config.DT_MOIST), "done")
