...
class TestCore:
    ...
    def __call__(self, constants: dict, cu_param_constants: dict, plume: str, **inputs):
        ...
        locals.cap_max_increment[:] = inputs["local_cap_max_increment"]
        locals.cap_max[:] = inputs["local_cap_max"]
        ...
        locals.environment_saturation_mixing_ratio_cloud_levels_forced[:] = inputs[
            "local_env_saturation_mixing_ratio_cloud_levels_forced"
        ]
        ...
        locals.environment_moist_static_energy_forced[:] = inputs[
            "local_env_moist_static_energy_forced"
        ]
        locals.moist_static_energy_origin_level_forced[:] = inputs[
            "local_moist_static_energy_origin_level_forced"
        ]
        locals.vapor_forced[:] = inputs["local_vapor_forced"]
        locals.environment_saturation_mixing_ratio_forced[:] = inputs[
            "local_env_saturation_mixing_ratio_forced"
        ]
        state.output.entrainment_rate[
            :, :, :, plume_dependent_constants.PLUME_INDEX
        ] = inputs["entrainment_rate"]
        locals.cloud_moist_static_energy_forced_transported[:] = inputs[
            "local_cloud_moist_static_energy_forced_transported"
        ]
        ...
        state.output.lcl_level[:, :, plume_dependent_constants.PLUME_INDEX] = (
            inputs["lcl_level"] - 1
        )
        ...
        locals.negative_buoyancy_depth[:] = inputs["local_negative_buoyancy_depth"]
        locals.frh_lfc[:] = inputs["local_frh_lfc"]
        ...
        locals.ocean_fraction[:] = inputs["ocean_fraction"]
        ...
        outputs = {
            ...
            + 1,
            "lcl_level": state.output.lcl_level.field[
                :, :, plume_dependent_constants.PLUME_INDEX
            ]
            + 1,
...
class TranslateGF2020_CumulusParameterization_ConvectiveCloudBaseLevel_shallow(
...
class TranslateGF2020_CumulusParameterization_ConvectiveCloudBaseLevel_mid(
    TranslateFortranData2Py
):
    def __init__(
        self,
...
class TranslateGF2020_CumulusParameterization_ConvectiveCloudBaseLevel_deep(
    TranslateFortranData2Py
):
    def __init__(
        self,
