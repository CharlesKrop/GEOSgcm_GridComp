...
class TestCore:
    ...
    def __call__(self, constants: dict, cu_param_constants: dict, plume: str, **inputs):
        ...
        state.output.updraft_lfc_level[:, :, plume_dependent_constants.PLUME_INDEX] = (
        ...
        state.output.precip[:, :, plume_dependent_constants.PLUME_INDEX] = inputs[
            "precip"
        ]
        locals.effective_condensate_to_fall_forced[:] = inputs[
            "local_effective_condensate_to_fall_forced"
        ]
        state.output.cloud_base_mass_flux_modified[
            :, :, plume_dependent_constants.PLUME_INDEX
        ] = inputs["cloud_base_mass_flux_modified"]
        state.output.scale_dependence_factor[
            :, :, plume_dependent_constants.PLUME_INDEX
        ] = inputs["scale_dependence_factor"]
        ...
        locals.del_cloud_liquid_cloud_ensemble[:] = inputs[
            "local_del_cloud_liquid_cloud_ensemble"
        ]
        locals.del_buoyancy_cloud_ensemble[:] = inputs[
            "local_del_buoyancy_cloud_ensemble"
        ]
        locals.del_convective_ice_cloud_ensemble[:] = inputs[
            "local_del_convective_ice_cloud_ensemble"
        ]
        locals.del_large_scale_ice_cloud_ensemble[:] = inputs[
            "local_del_large_scale_ice_cloud_ensemble"
        ]
        locals.del_convective_liquid_cloud_ensemble[:] = inputs[
            "local_del_convective_liquid_cloud_ensemble"
        ]
        locals.del_large_scale_liquid_cloud_ensemble[:] = inputs[
            "local_del_large_scale_liquid_cloud_ensemble"
        ]
        locals.del_convective_cloud_fraction_cloud_ensemble[:] = inputs[
            "local_del_convective_cloud_fraction_cloud_ensemble"
        ]
        locals.del_large_scale_cloud_fraction_cloud_ensemble[:] = inputs[
            "local_del_large_scale_cloud_fraction_cloud_ensemble"
        ]
        state.output.dtdt[:, :, :, plume_dependent_constants.PLUME_INDEX] = inputs[
            "dtdt"
        ]
        state.output.dvapordt[:, :, :, plume_dependent_constants.PLUME_INDEX] = inputs[
            "dvapordt"
        ]
        state.output.dcloudicedt[:, :, :, plume_dependent_constants.PLUME_INDEX] = (
            inputs["dcloudicedt"]
        )
        state.output.dudt[:, :, :, plume_dependent_constants.PLUME_INDEX] = inputs[
            "dudt"
        ]
        state.output.dvdt[:, :, :, plume_dependent_constants.PLUME_INDEX] = inputs[
            "dvdt"
        ]
        state.output.dbuoyancydt[:, :, :, plume_dependent_constants.PLUME_INDEX] = (
            inputs["dbuoyancydt"]
        )
        state.output.dconvectiveicedt[
            :, :, :, plume_dependent_constants.PLUME_INDEX
        ] = inputs["dconvectiveicedt"]
        state.output.dlargescaleicedt[
            :, :, :, plume_dependent_constants.PLUME_INDEX
        ] = inputs["dlargescaleicedt"]
        state.output.dconvectiveliquiddt[
            :, :, :, plume_dependent_constants.PLUME_INDEX
        ] = inputs["dconvectiveliquiddt"]
        state.output.dlargescaleliquiddt[
            :, :, :, plume_dependent_constants.PLUME_INDEX
        ] = inputs["dlargescaleliquiddt"]
        state.output.dconvectivecloudfractiondt[
            :, :, :, plume_dependent_constants.PLUME_INDEX
        ] = inputs["dconvectivecloudfractiondt"]
        state.output.dlargescalecloudfractiondt[
            :, :, :, plume_dependent_constants.PLUME_INDEX
        ] = inputs["dlargescalecloudfractiondt"]
        locals.mass_flux_ensemble[:] = inputs["local_mass_flux_ensemble"][:, :, 0:16]
        locals.precipitation_ensemble[:] = inputs["local_precipitation_ensemble"][
            :, :, 0:16
        ]
        ...
        outputs = {
            ...
            "precip": state.output.precip.field[
                :, :, plume_dependent_constants.PLUME_INDEX
            ],
            ...
            "dtdt": state.output.dtdt.field[
                :, :, :, plume_dependent_constants.PLUME_INDEX
            ],
            "dvapordt": state.output.dvapordt.field[
                :, :, :, plume_dependent_constants.PLUME_INDEX
            ],
            "dcloudicedt": state.output.dcloudicedt.field[
                :, :, :, plume_dependent_constants.PLUME_INDEX
            ],
            "dudt": state.output.dudt.field[
                :, :, :, plume_dependent_constants.PLUME_INDEX
            ],
            "dvdt": state.output.dvdt.field[
                :, :, :, plume_dependent_constants.PLUME_INDEX
            ],
            "dbuoyancydt": state.output.dbuoyancydt.field[
                :, :, :, plume_dependent_constants.PLUME_INDEX
            ],
...
class TranslateGF2020_CumulusParameterization_EnsembleOutputAndFeedback_shallow(
...
class TranslateGF2020_CumulusParameterization_EnsembleOutputAndFeedback_mid(
    TranslateFortranData2Py
):
    def __init__(
        self,
...
class TranslateGF2020_CumulusParameterization_EnsembleOutputAndFeedback_deep(
    TranslateFortranData2Py
):
    def __init__(
        self,
